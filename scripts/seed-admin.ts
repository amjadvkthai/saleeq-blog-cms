...
async function seed() {
  ...
  try {
    ...
    const existing = await env.DB.prepare('SELECT id FROM auth_user WHERE email = ?').bind('saleeq@blog.com').first()
    if (existing) {
    ...
    const passwordHash = await hashPassword('saleeq@222')
    ...
    await env.DB.batch([
      ...
      ).bind(odid, 'saleeq@blog.com', 'Admin', 'User', 'admin', 1, nowMs, nowMs, 'Admin User'),
      env.DB.prepare(
    ...
    console.log(`  Email: saleeq@blog.com`)
